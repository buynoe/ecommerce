...
export default function CartBadge({ slug, storeId }: { slug: string; storeId: string }) {
  ...
  return (
    <Link href={`/store/${slug}/cart`} aria-label="Cart"
      ...
      {count > 0 && (
        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-green-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 leading-none">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </Link>
