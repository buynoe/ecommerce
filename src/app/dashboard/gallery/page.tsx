...
import { Upload, Trash2, Camera, Clipboard, Maximize2, Cloud, Home, FolderOpen, Folder, Check, X } from "lucide-react";
    ...
    <div className="flex h-full -mt-6 -mx-6 min-h-screen">
      ...
      <aside className="w-56 shrink-0 border-r border-gray-200 bg-gray-50 flex flex-col pt-6">
        <div className="px-4 mb-4">
          ...
          {showNewFolder ? (
            <div className="flex gap-1 mb-2">
              ...
              <button onClick={createFolder} className="btn-brand px-2 py-1 rounded text-xs flex items-center"><Check className="w-3 h-3" /></button>
              <button onClick={() => setShowNewFolder(false)} className="text-gray-400 px-2 py-1 flex items-center"><X className="w-3 h-3" /></button>
            </div>
          ) : (
        ...
        <nav className="flex-1 overflow-y-auto px-2 space-y-0.5">
          {[
            { id: null, label: "All Images", icon: <Home className="w-4 h-4 shrink-0" /> },
            { id: "null", label: "Uncategorized", icon: <FolderOpen className="w-4 h-4 shrink-0" /> },
          ].map(item => (
          ...
          {folders.map(f => (
            <button key={f.id} onClick={() => setActiveFolderId(f.id)}
              ...
              <span className="flex items-center gap-2 min-w-0"><Folder className="w-4 h-4 shrink-0" /><span className="truncate">{f.name}</span></span>
                ...
                {assets.map(asset => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSel && <div className="absolute top-1.5 right-1.5 w-5 h-5 bg-green-500 rounded-full flex items-center justify-center shadow"><Check className="w-3 h-3 text-white" /></div>}
          ...
          {detail && (
            <div className="w-64 shrink-0 border-l border-gray-200 bg-gray-50 overflow-y-auto flex flex-col">
              <div className="p-3 border-b border-gray-200 flex items-center justify-between">
                ...
                <button onClick={() => setDetail(null)} className="text-gray-400 hover:text-gray-600 flex items-center"><X className="w-4 h-4" /></button>
              </div>
              <div className="p-4 space-y-4">
                ...
                <div className="text-xs space-y-1 text-gray-500">
                  ...
                  {detail.width && <p className="flex items-center gap-1"><Maximize2 className="w-3 h-3" /> {detail.width} × {detail.height}px</p>}
                  <p>{formatBytes(detail.size)}</p>
                  {detail.publicId && <p className="text-[#ec1f78] font-medium flex items-center gap-1"><Cloud className="w-3 h-3" /> Cloudinary</p>}
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(detail.url)} className="border border-gray-200 rounded-lg px-2 flex items-center hover:bg-gray-100" title="Copy URL"><Clipboard className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
                <button onClick={async () => { await fetch("/api/gallery/assets", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: detail.id }) }); setDetail(null); setAssets(a => a.filter(x => x.id !== detail.id)); loadFolders(); }}
                  className="w-full py-2 text-xs text-red-500 border border-red-200 rounded-lg hover:bg-red-50 font-semibold flex items-center justify-center gap-1.5">
